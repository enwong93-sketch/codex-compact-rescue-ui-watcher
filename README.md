...
- Completion is not guessed by a fixed timer. The watcher records existing compact markers, waits for a newly visible completion marker, and confirms the active compacting status is gone before switching back.
...
- If the watcher starts while the thread is already on GPT-5.4-Mini, it will not infer completion from old visible `上下文已自動精簡` markers. This avoids stopping an active compact in the middle.
