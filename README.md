...
- The watcher suppresses the same visible compact error for 15 minutes after a successful recovery, so old error text does not trigger another round. Active compacting status text is only ignored while that same UI element remains visible; once it disappears, a later status marker can start a real new round.
