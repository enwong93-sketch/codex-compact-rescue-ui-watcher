...
- `-GoalResumeWaitSeconds`: after final `繼續`, scan this long for a paused `/goal` row and click its resume/play button. Default: 30.
...
- `-NoGoalResume`: disable the paused `/goal` resume click after final `繼續`.
...
- Switching models is retried. Automatic recovery now uses keyboard navigation first, then anchored clicks; it does not use the UIA menu-scan mouse path during normal recovery.
- The retained menu-scan helper scans only visible menu items instead of the full desktop UI tree, but normal recovery avoids that path because some Windows UI Automation providers can still hang after a menu opens.
- Every fallback must confirm the target model before returning success. If Codex selects another model such as GPT-5.2, the watcher keeps retrying and will not continue as if GPT-5.4-Mini was selected.
